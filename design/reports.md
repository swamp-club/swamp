...
Swamp ships two built-in reports that run by default for every model method run
and workflow run, providing a basic summary of what happened. They are registered
at startup via a side-effect barrel import in the CLI.

### `@swamp/method-summary`

- **Scope**: `method`
- **Labels**: `["summary"]`
- **Output**: Status, model info, arguments, and data produced

Runs automatically for every `model method run`. The report is injected into the
candidate set alongside any model-type default reports.

### `@swamp/workflow-summary`

- **Scope**: `workflow`
- **Labels**: `["summary"]`
- **Output**: Status, step table with failures highlighted first, and data
  produced per step

Runs automatically for every `workflow run`. The report is injected into the
candidate set for workflow-scope report execution.

### Disabling Built-in Reports

Users suppress built-in reports using the existing `reports.skip` mechanism:
...
```bash
swamp model method run my-server deploy --skip-report @swamp/method-summary
swamp workflow run deploy --skip-reports
```

See `src/domain/reports/builtin/` for the implementation.
