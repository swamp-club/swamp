...
`${{ model.<name>.resource.<specName>.attributes.<field> }}`, the workflow engine
automatically:
...
All model data outputs are accessed via `model.<name>.resource.<specName>.attributes.<field>`:
