...
export async function* workflowRun(
  ...
  for await (const event of deps.executionService.execute(ctx, workflow, input.args)) {
    yield { kind: "stepCompleted", stepName: event.stepName, result: event.result };
  }
...
The chain `MethodExecutionEvent → onEvent → WorkflowExecutionEvent →
WorkflowRunEvent` keeps each layer ignorant of layers above it.
