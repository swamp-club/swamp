...
example, expired or wrong datastore credentials), `mint`, `rotate` and `reveal`
fail with that underlying error. Fix the datastore access and rerun; do not
create a vault named `_token-secrets`. For headless deployments, mint inside the
serve process's own environment — e.g. via `kubectl exec` — not on the
operator's laptop.
