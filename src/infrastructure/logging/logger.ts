...
export async function initializeLogging(
  ...
  if (options.debugLogs) {
    ...
    loggers.push({
      category: [],
      lowestLevel: "debug",
      sinks: options.jsonMode
        ? ["file"]
        : ["file", options.prettyOutput ? "pretty" : "console"],
    });

    if (options.jsonMode) {
      // In debug+JSON mode, also send fatal messages to the JSON error sink
      // so errors appear as structured JSON on stderr.
      loggers.push({
        category: [],
        lowestLevel: "fatal",
        sinks: ["jsonError"],
