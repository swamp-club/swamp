...
Deno.test("SwampClubClient - updateApiKey succeeds on 200", async () => {
  ...
  try {
    const client = new SwampClubClient(`http://localhost:${mock.port}`);
    await client.updateApiKey("my-api-key", "key-1", false);
...
Deno.test("SwampClubClient - deleteApiKey succeeds on 200", async () => {
  ...
  try {
    const client = new SwampClubClient(`http://localhost:${mock.port}`);
    await client.deleteApiKey("my-api-key", "key-1");
    assertEquals(capturedApiKey, "my-api-key");
  } finally {
    await mock.shutdown();
  }
...
Deno.test("SwampClubClient - createApiKey with api-key auth method sends x-api-key header", async () => {
  let capturedApiKey = "";
  let capturedAuth = "";
  const mock = startMockServer((req) => {
    capturedApiKey = req.headers.get("x-api-key") ?? "";
    capturedAuth = req.headers.get("authorization") ?? "";
    return Response.json({ id: "key-2", key: "swamp_xyz789" });
  });
  try {
    const client = new SwampClubClient(`http://localhost:${mock.port}`);
    const result = await client.createApiKey("my-api-key", "test-key", "api-key");
    assertEquals(result.id, "key-2");
    assertEquals(result.key, "swamp_xyz789");
    assertEquals(capturedApiKey, "my-api-key");
