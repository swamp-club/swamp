...
export class YamlWorkflowRunRepository implements WorkflowRunRepository {
  ...
  async findById(
    ...
    try {
      for await (const entry of Deno.readDir(dir)) {
        if (entry.isFile && entry.name.includes(runId) && entry.name.endsWith(".yaml")) {
  ...
  async findAllByWorkflowId(workflowId: WorkflowId): Promise<WorkflowRun[]> {
    ...
    try {
      for await (const entry of Deno.readDir(dir)) {
        if (entry.isFile && entry.name.startsWith("workflow-run-") && entry.name.endsWith(".yaml")) {
          const path = join(dir, entry.name);
          const content = await Deno.readTextFile(path);
          const data = parseYaml(content) as WorkflowRunData;
  ...
  async findLatestByWorkflowId(workflowId: WorkflowId): Promise<WorkflowRun | null> {
  ...
  getPath(workflowId: WorkflowId, runId: WorkflowRunId): string {
    ...
    return join(this.getRunsDir(workflowId), `workflow-run-${runId}-${timestamp}.yaml`);
  }
