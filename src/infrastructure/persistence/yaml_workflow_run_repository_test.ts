...
Deno.test("YamlWorkflowRunRepository.nextId returns unique IDs", async () => {
  await withTempDir(async (dir) => {
    const repo = new YamlWorkflowRunRepository(dir);
...
Deno.test("YamlWorkflowRunRepository preserves run state", async () => {
  await withTempDir(async (dir) => {
    ...
    assertEquals(loaded!.getJob("job1")?.getStep("step1")?.output, { result: "success" });
  });
});
...
Deno.test("YamlWorkflowRunRepository preserves error information", async () => {
  await withTempDir(async (dir) => {
    ...
    assertEquals(loaded!.getJob("job1")?.getStep("step1")?.error, "Something went wrong");
  });
});
