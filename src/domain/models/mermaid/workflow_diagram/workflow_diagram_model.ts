 ...
 */
export type MermaidWorkflowInputAttributes = z.infer<typeof MermaidWorkflowInputAttributesSchema>;
 ...
 */
export const MERMAID_WORKFLOW_MODEL_TYPE = ModelType.create("mermaid/workflow-diagram");
 ...
 */
function getStatusColor(status: string, colorScheme: Record<string, string>): string {
...
function generateMermaidDiagram(
  ...
  for (const job of execution.jobs) {
    ...
    if (options.includeSteps && job.steps.length > 0) {
      ...
      for (const step of job.steps) {
        ...
        styling.push(`    classDef ${stepId}_class fill:${getStatusColor(step.status, options.colorScheme)}`);
      ...
      for (let i = 0; i < stepNodes.length - 1; i++) {
        const hasExplicitDeps = job.steps[i + 1].dependsOn && job.steps[i + 1].dependsOn!.length > 0;
        if (!hasExplicitDeps) {
    ...
    // Add job styling
    styling.push(`    classDef ${jobId}_class fill:${getStatusColor(job.status, options.colorScheme)}`);
  ...
  styling.push(`    classDef ${endId}_class fill:${getStatusColor(execution.status, options.colorScheme)}`);
