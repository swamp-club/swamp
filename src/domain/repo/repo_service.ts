...
- **Look for aggregator methods first**: when the question is "is X healthy?" / "what's the state of Y?" / "list all results across instances", check whether the extension exposes a single method that returns the answer. Common naming patterns: \`health\`, \`summary\`, \`*All\` (e.g. \`getRolloutStatusAll\`), \`diagnose*\`. One call beats reconstructing the answer from primitives. Find them with \`--methods\`.
