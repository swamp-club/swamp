...
- **Discover methods compactly**: \`swamp model type describe <type> --methods\` returns one line per method (name + description), no schemas. Once you've picked a method, \`swamp model type describe <type> --method <name>\` returns just that method's argument schema (~10× smaller than the full \`--json\` dump). Reach for full \`--json\` only when you need every method's schema at once.
- **Default to \`data list --content\` when reading model data**: \`swamp data list <model> --content --json\` returns metadata AND parsed content for every record in one call. \`swamp data get <model> <name>\` is for the specific case where you already know the exact record name AND don't want anything else from that model — usually NOT the right first move when exploring or diagnosing, and N sequential \`data get\` calls are almost always wrong.
...
- **Search for existing workflows before writing one**: \`swamp workflow search [query] --json\` lists everything available. The extension you're using probably ships a workflow for the task you're trying to do — running it is one CLI call vs. an N-cycle write/validate/run loop. Pulling a community extension also pulls its workflows.
- **Look for aggregator methods first**: when the question is "is X healthy?" / "what's the state of Y?" / "list all results across instances", check whether the extension exposes a single method that returns the answer. Common naming hints: \`health\`, \`summary\`, \`status\`, \`diagnose\`, or any name that sounds like a roll-up rather than a primitive. One call beats reconstructing the answer from primitives. Find them with \`--methods\`.
- **If you're fanning out method calls across instances of one model, stop and check for a batch variant first**. \`swamp model method run X foo --input id=a\` × N is N times the dispatch overhead and N times the agent reasoning. If no batch variant exists, write a workflow with \`forEach\` once instead — one workflow run beats N method runs.
