...
Deno.test("validates simple valid workflow", async () => {
  ...
  assertEquals(failed.length, 0, `Failed validations: ${failed.map((r) => r.name).join(", ")}`);
});
...
Deno.test("fails on duplicate job names", async () => {
  ...
  const results = await service.validate(workflow);
  const uniqueJobNamesResult = results.find((r) => r.name === "Unique job names");
...
Deno.test("fails on duplicate step names within job", async () => {
  ...
  const results = await service.validate(workflow);
  const stepNamesResult = results.find((r) => r.name.includes("Unique step names"));
...
Deno.test("fails on invalid job dependency reference", async () => {
  ...
  const results = await service.validate(workflow);
  const jobRefsResult = results.find((r) => r.name === "Valid job dependency references");
...
Deno.test("fails on invalid step dependency reference", async () => {
  ...
  const results = await service.validate(workflow);
  const stepRefsResult = results.find((r) => r.name.includes("Valid step dependency references"));
...
Deno.test("fails on cyclic job dependencies", async () => {
  ...
  // Create cycle: a -> b -> a
  (jobA as unknown as { _dependsOn: { job: string; condition: TriggerCondition }[] })._dependsOn = [
    { job: "b", condition: TriggerCondition.always() },
  ];
  (jobB as unknown as { _dependsOn: { job: string; condition: TriggerCondition }[] })._dependsOn = [
  ...
  const results = await service.validate(workflow);
  const cycleResult = results.find((r) => r.name === "No cyclic job dependencies");
...
Deno.test("fails on cyclic step dependencies", async () => {
  ...
  // Create cycle: a -> b -> a
  (stepA as unknown as { _dependsOn: { step: string; condition: TriggerCondition }[] })._dependsOn = [
    { step: "b", condition: TriggerCondition.always() },
  ];
  (stepB as unknown as { _dependsOn: { step: string; condition: TriggerCondition }[] })._dependsOn = [
  ...
  const results = await service.validate(workflow);
  const cycleResult = results.find((r) => r.name.includes("No cyclic step dependencies"));
  assertEquals(cycleResult?.passed, false);
  assertEquals(cycleResult?.error?.includes("Cyclic"), true);
});
...
Deno.test("passes all validations for complex valid workflow", async () => {
      ...
      Job.create({
        ...
        steps: [
          ...
          Step.create({
            ...
            dependsOn: [
              { step: "checkout", condition: TriggerCondition.succeeded("checkout") },
            ],
          }),
        ],
      }),
  ...
  const results = await service.validate(workflow);
  const failed = results.filter((r) => !r.passed);
  assertEquals(failed.length, 0, `Failed: ${failed.map((r) => `${r.name}: ${r.error}`).join("; ")}`);
});
