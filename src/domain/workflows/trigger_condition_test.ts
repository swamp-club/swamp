...
function createContext(
  statuses: Record<string, "pending" | "running" | "succeeded" | "failed" | "skipped">,
...
Deno.test("complex nested condition evaluates correctly", () => {
  ...
  assertEquals(
    cond.evaluate(createContext({ step1: "succeeded", step2: "succeeded", step3: "succeeded" })),
    true,
  );

  // Test case: step3 failed
  assertEquals(
    cond.evaluate(createContext({ step1: "failed", step2: "failed", step3: "failed" })),
    true,
  );

  // Test case: neither condition met
  assertEquals(
    cond.evaluate(createContext({ step1: "succeeded", step2: "failed", step3: "succeeded" })),
    false,
  );
});
