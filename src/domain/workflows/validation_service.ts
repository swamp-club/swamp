 ...
 */
export class DefaultWorkflowValidationService implements WorkflowValidationService {
  ...
  private validateUniqueStepNames(workflow: Workflow): WorkflowValidationResult[] {
    const results: WorkflowValidationResult[] = [];
    ...
    for (const job of workflow.jobs) {
      ...
      if (duplicates.length > 0) {
        ...
        results.push(
          WorkflowValidationResult.pass(`Unique step names in job '${job.name}'`),
        );
      }
    }
  ...
  private validateJobDependencyRefs(workflow: Workflow): WorkflowValidationResult {
  ...
  private validateStepDependencyRefs(workflow: Workflow): WorkflowValidationResult[] {
    const results: WorkflowValidationResult[] = [];
  ...
  private validateNoStepCycles(workflow: Workflow): WorkflowValidationResult[] {
    ...
    for (const job of workflow.jobs) {
      if (job.steps.length === 0) {
        results.push(
          WorkflowValidationResult.pass(`No cyclic step dependencies in job '${job.name}'`),
        );
        continue;
      }
