...
Deno.test("methodSummaryReport: succeeded method with args and data handles", async () => {
  ...
  // Markdown checks
  assertStringIncludes(result.markdown, "# Method Summary");
  assertStringIncludes(result.markdown, "**Status**: succeeded");
  assertStringIncludes(result.markdown, "**Model**: my-server (def-123)");
  assertStringIncludes(result.markdown, "**Type**: server");
  assertStringIncludes(result.markdown, "**Method**: deploy");
  assertStringIncludes(result.markdown, '"region": "us-east-1"');
  assertStringIncludes(result.markdown, '"force": true');
  assertStringIncludes(result.markdown, "| output.json | file | 256 |");
  ...
  assertEquals(result.json.dataProduced, [
    { name: "output.json", kind: "file", size: 256 },
  ]);
});
...
Deno.test("methodSummaryReport: empty globalArgs shows no global arguments", async () => {
  const ctx = makeMethodContext({ globalArgs: {} });

  const result = await methodSummaryReport.execute(ctx);

  assertStringIncludes(result.markdown, "No global arguments.");
});

Deno.test("methodSummaryReport: empty methodArgs shows no method arguments", async () => {
  const ctx = makeMethodContext({ methodArgs: {} });

  const result = await methodSummaryReport.execute(ctx);

  assertStringIncludes(result.markdown, "No method arguments.");
});

Deno.test("methodSummaryReport: undefined globalArgs shows no global arguments", async () => {
  const ctx = makeMethodContext({
    // deno-lint-ignore no-explicit-any
...
Deno.test("methodSummaryReport: undefined methodArgs shows no method arguments", async () => {
  const ctx = makeMethodContext({
    // deno-lint-ignore no-explicit-any
...
Deno.test("methodSummaryReport: empty dataHandles shows no data produced", async () => {
  const ctx = makeMethodContext({ dataHandles: [] });

  const result = await methodSummaryReport.execute(ctx);

  assertStringIncludes(result.markdown, "No data produced.");
  assertEquals(result.json.dataProduced, []);
});
...
Deno.test("methodSummaryReport: JSON output structure matches expected shape", async () => {
  ...
  assertEquals(Object.keys(items[0]).sort(), ["kind", "name", "size"]);
});
