...
Deno.test("reportDescribe - returns report definition details", async () => {
  ...
  if (last.kind === "completed") {
    ...
    assertEquals(last.data.builtin, false);
  }
});

Deno.test("reportDescribe - marks @swamp/ reports as builtin", async () => {
  const ctx = createLibSwampContext();
  const deps = makeDeps({ "@swamp/test-report": testReport });

  const events = await collect(
    reportDescribe(ctx, deps, "@swamp/test-report"),
  );
  const last = events[events.length - 1];

  assertEquals(last.kind, "completed");
  if (last.kind === "completed") {
    assertEquals(last.data.builtin, true);
  }
});
