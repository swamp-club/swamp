...
export interface ModelMethodRunDeps {
  ...
  unregisterLogSink: (prefix: string[]) => void;
  createSecretRedactor: () => SecretRedactor;
  computeLogFilePath: (
    modelType: ModelType,
    methodName: string,
    definitionId: string,
...
export async function* modelMethodRun(
  ...
  // --- Set up logging ---
  const redactor = deps.createSecretRedactor();
  const logFilePath = deps.computeLogFilePath(
    modelType,
    input.methodName,
    definition.id,
  );
  const runLogCategory: string[] = [];
  const logBoundary = swampPath(deps.repoDir);
  await deps.registerLogSink(
    runLogCategory,
    logFilePath,
    redactor,
    logBoundary,
  ...
  try {
    ...
    const evaluationService = deps.createEvaluationService(deps.repoDir);
    ...
    if (input.lastEvaluated) {
      ...
      if (!lastEval) {
        yield {
          kind: "error",
          error: noEvaluatedDefinition(definition.name, input.methodName),
        };
        return;
      }
    ...
    deps.unregisterLogSink(runLogCategory);
  }
}
...
export function noEvaluatedDefinition(
  ...
  return {
    ...
    message: `No previously evaluated definition found for "${name}".\n\n` +
      ...
      `  swamp model method run ${name} ${methodName}`,
  };
}
