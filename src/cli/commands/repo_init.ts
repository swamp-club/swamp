...
export const repoInitCommand = new Command()
  ...
  .option(
    "--server <url:string>",
    "Default serve URL for this repository — stored in .swamp.yaml as serverAddress (env: SWAMP_SERVE_URL overrides at runtime). A URL with a username, password, query string or fragment is refused; give remote commands the token with --token, SWAMP_SERVER_TOKEN or swamp auth server-login",
  )
