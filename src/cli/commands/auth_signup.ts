...
export const authSignupCommand = new Command()
  .name("signup")
  .description("Create a new swamp.club account")
  .option("--email <email:string>", "Email address")
  .option("--password <password:string>", "Password")
  .option("--name <name:string>", "Display name")
  .action(async function (options: AnyOptions) {
    const ctx = createContext(options as GlobalOptions, ["auth", "signup"]);
    ...
    if (!email || !password || !name) {
      if (!isStdinTty() || ctx.outputMode === "json") {
        throw new UserError(
          "--email, --password, and --name are required in non-interactive mode",
        );
      }

      if (!name) {
        name = await readLine("Display name: ");
