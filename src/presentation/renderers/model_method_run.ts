  ...
  handlers(): EventHandlers<ModelMethodRunEvent> {
    return {
      ...
      completed: (e) => {
        ...
        // Preserve backward-compatible JSON output shape
        const data: Record<string, unknown> = {
          modelId: e.run.modelId,
          modelName: e.run.modelName,
          type: e.run.modelType,
          methodName: e.run.methodName,
        };
        if (e.run.dataArtifacts.length > 0) {
          data.data = {
            id: e.run.dataArtifacts[0].id,
            path: e.run.dataArtifacts[0].path,
            attributes: e.run.dataArtifacts[0].attributes,
          };
        }
        if (e.run.dataArtifacts.length > 1) {
          data.logs = e.run.dataArtifacts.slice(1).map((a) => ({
            id: a.id,
            path: a.path,
