...
Deno.test("JsonModelMethodRunRenderer - completed serializes backward-compatible JSON", async () => {
  const logs: string[] = [];
  const originalLog = console.log;
  console.log = (msg: string) => logs.push(msg);
  ...
  try {
    ...
    assertEquals(parsed.type, "test/type"); // NOT "modelType"
    assertEquals(parsed.methodName, "run");
    assertEquals(parsed.data.id, "art-1");
    assertEquals(parsed.data.path, "/data/result/v1");
    assertEquals(parsed.data.attributes.exitCode, 0);
  } finally {
    console.log = originalLog;
  }
});
