...
Deno.test("parseAuditConfig: returns null when no audit block", () => {
  assertEquals(parseAuditConfig(null), null);
  assertEquals(parseAuditConfig({}), null);
  assertEquals(parseAuditConfig({ audit: {} }), null);
  assertEquals(parseAuditConfig({ audit: { stores: [] } }), null);
});

Deno.test("parseAuditConfig: parses minimal config with defaults", () => {
  const result = parseAuditConfig({
    audit: { stores: [{ target: "default" }] },
  });
  assertEquals(result?.stores, [{ target: "default" }]);
  assertEquals(result?.batchSize, 100);
  assertEquals(result?.flushIntervalMs, 5_000);
});

Deno.test("parseAuditConfig: parses custom batch-size and flush-interval", () => {
  const result = parseAuditConfig({
    audit: {
  ...
  assertEquals(result?.batchSize, 50);
  assertEquals(result?.flushIntervalMs, 10_000);
});

Deno.test("parseAuditConfig: parses ms and m flush-interval", () => {
  assertEquals(
    parseAuditConfig({
      audit: { stores: [{ target: "a" }], "flush-interval": "500ms" },
    })?.flushIntervalMs,
    500,
  );
  assertEquals(
    parseAuditConfig({
      audit: { stores: [{ target: "a" }], "flush-interval": "2m" },
    })?.flushIntervalMs,
    120_000,
  );
});

Deno.test("loadServeConfig: rejects invalid audit.stores type", () => {
  const dir = Deno.makeTempDirSync();
  try {
    ...
    Deno.writeTextFileSync(
      path,
      stringifyYaml({ audit: { stores: "not-an-array" } }),
    );
    assertThrows(
...
Deno.test("loadServeConfig: rejects invalid audit.flush-interval format", () => {
  const dir = Deno.makeTempDirSync();
  try {
    const path = join(dir, ".swamp", "serve.yaml");
    Deno.mkdirSync(join(dir, ".swamp"), { recursive: true });
...
Deno.test("loadServeConfig: rejects audit store without target", () => {
  const dir = Deno.makeTempDirSync();
  try {
    const path = join(dir, ".swamp", "serve.yaml");
    Deno.mkdirSync(join(dir, ".swamp"), { recursive: true });
